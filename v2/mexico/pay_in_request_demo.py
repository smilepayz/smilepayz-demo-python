...
from v2.mexico.bean.TradePayInReq import TradePayInReq
...
def transaction_pay_in(env, merchant_id, merchant_secret, private_key, payment_method, amount):
    ...
    assert result['status'] in ['PROCESSING', 'SUCCESS'], f"状态异常：{result['status']}"
    ...
    assert result['money']['currency'] == 'MXN', f"币种错误，返回币种：{result['money']['currency']}"
    assert 'channel' in result, "返回结果缺少channel字段"
    assert 'paymentMethod' in result['channel'], "channel字段缺少paymentMethod"
    assert result['channel']['paymentMethod'] == 'SPEI', f"支付方式不匹配，返回支付方式：{result['channel']['paymentMethod']}"
