...
def pay_out_request_demo(env, merchant_id, merchant_secret, private_key, payment_method, amount, cash_account,ifsc_code):
    global request_path
    if env == "production":
        # production
    ...
    # merchantReq
    merchant_req = MerchantReq(merchant_id, "rain", None)

    #receiver
    receiver_req = ReceiverReq(name='sonia')
    ...
    pay_in_req = TradePayoutReq(payment_method, None, receiver_req, cash_account, ifsc_code, merchant_order_no[:32],
                                ...
                                'https://test', AreaEnum.INDIA.code)
    ...
    assert result.get('money', {}).get('currency') == 'INR', f"币种错误，currency={result.get('money', {}).get('currency')}"
    assert isinstance(result.get('money', {}).get('amount'), (int, float)), "金额不合法"
    assert result.get('channel', {}).get('paymentMethod') == 'BOBAK', f"支付方式错误，paymentMethod={result.get('channel', {}).get('paymentMethod')}"
