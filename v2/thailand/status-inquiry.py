...
def status_inquiry(env, merchant_id, merchant_secret, private_key, trade_type, trade_no, order_no):
    ...
    assert result.get('paymentMethod') == 'QRPAY', f"支付方式错误，paymentMethod={result.get('paymentMethod')}"
    assert result.get('money', {}).get('currency') == 'THB', f"币种错误，currency={result.get('money', {}).get('currency')}"
...
if __name__ == '__main__':
    ...
    trade_type = "1"
    trade_no = "1112015825061712423216061"
    order_no = ""
