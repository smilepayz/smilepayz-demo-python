...
from v2.indonesia.bean.PayerReq import PayerReq
...
def transaction_pay_in(env, merchant_id, merchant_secret, private_key, payment_method, amount):
    ...
    # merchantReq
    merchant_req = MerchantReq(merchant_id, "", None,'2019002')
    # merchant_req = MerchantReq(merchant_id, "", None,'20190002)'#子商户


    pay_in_req = TradePayInReq(payment_method, payer_req, None, None, merchant_order_no[:32], purpose,
                               None,
                               None,
                               None, None, None, money_req, merchant_req, "http://www.ningning.com",
                               "http://www.dai.com")
    ...
    assert result['money']['currency'] == 'IDR', f"币种错误，返回币种：{result['money']['currency']}"
    assert 'channel' in result, "返回结果缺少channel字段"
    assert 'paymentMethod' in result['channel'], "channel字段缺少paymentMethod"
    assert result['channel']['paymentMethod'] == 'Qris', f"支付方式不匹配，返回支付方式：{result['channel']['paymentMethod']}"
