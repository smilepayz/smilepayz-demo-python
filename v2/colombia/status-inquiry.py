...
from v2.colombia.bean.OrderStatusInquiry import OrderStatusInquiry
...
def status_inquiry(env,merchant_id,merchant_secret,private_key,trade_type,trade_no,order_no):
    ...
    assert result.get('paymentMethod') == 'CASHIER_CO', f"支付方式错误，paymentMethod={result.get('paymentMethod')}"
    assert result.get('money', {}).get('currency') == 'COP', f"币种错误，currency={result.get('money', {}).get('currency')}"
...
if __name__ == '__main__':
    ...
    trade_type = "1"
    trade_no = ""
    order_no = "2320158250617100532336"
